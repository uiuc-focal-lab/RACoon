...
def process_file(file, eps):
    ...
    for line in file:
        ...
        if tokens[0] == 'individual':
            if tokens[1] == 'acc':
                individual =  round((100.0 - float(tokens[2])) / 5, 0)
            if tokens[1] == 'time':
                individual_time =  float(tokens[2])
            continue
        if tokens[0] == 'baseline':
            if tokens[1] == 'acc':
                IO =  round((100.0 - float(tokens[2])) / 5, 0)
            if tokens[1] == 'time':
        ...
        if tokens[0] == 'IndivRefine':
            if tokens[1] == 'acc':
                individual_refinement =  round((100.0 - float(tokens[2])) / 5, 0)
            if tokens[1] == 'time':
                individual_refinement_time =  float(tokens[2])
            continue    
        if tokens[0] == 'IndivRefineMILP':
            if tokens[1] == 'acc':
                individual_refinement_MILP =  round((100.0 - float(tokens[2])) / 5, 0)
            if tokens[1] == 'time':
                individual_refinement_MILP_time =  float(tokens[2])
            continue    
        if tokens[0] == 'CrossEx':
            if tokens[1] == 'acc':
                cross_ex =  round((100.0 - float(tokens[2])) / 5, 0)
            if tokens[1] == 'time':
                cross_ex_time =  float(tokens[2])
            continue
        if tokens[0] == 'final':
            if tokens[1] == 'acc':
                cross_ex_MILP =  round((100.0 - float(tokens[2])) / 5, 0)
            if tokens[1] == 'time':
...
def main():
    ...
    thresholds[config.MNIST_CROWN_IBP] = [None, 0.1503]
    thresholds[config.MNIST_FFN_01] = [None, None]
    thresholds[config.MNIST_CROWN_IBP_MED] = [None, 0.3503]
    thresholds[config.MNIST_CONV_PGD] = [None, 0.1503]
    thresholds[config.MNIST_CONV_SMALL] = [None, 0.1503]
    thresholds[config.MNIST_CONV_BIG] = [None, 0.2003]

    for net_name in mnist_net_names:
       prop_count_result =  read_files(net_name=net_name, source_directory='./icml_hamming/', thershold=None)
