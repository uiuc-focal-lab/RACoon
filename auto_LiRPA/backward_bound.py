...
def backward_general(
        ...
        need_A_only=False, unstable_idx=None, unstable_size=0, update_mask=None, 
        verbose=True, multiple_execution=False):
    if verbose:
    ...
    # print(f'Final Node name {node.name}')
    ...
    while len(queue) > 0:
        ...
        # if l.name == '/input.12':
        #     print(f'lA shape {l.lA.shape}')
        #     print(f'uA shape {l.uA.shape}')
    ...
    if not multiple_execution:
        ...
        lb, ub = concretize(
            lb, ub, node, self.root, batch_size, output_dim,
            bound_lower, bound_upper, average_A=average_A, res_dict=res_dict)
        self.A_dict["final_coef"] = res_dict
    # TODO merge into `concretize`
...
def concretize(lb, ub, node, root, batch_size, output_dim, bound_lower=True, 
               bound_upper=True, average_A=False, res_dict=None):
    count = 0
    for i in range(len(root)):
        if root[i].lA is None and root[i].uA is None: continue
        count += 1
        if count > 1 and res_dict is not None:
            raise ValueError("Cross execution param learning not supported for multiple roots")
        ...
        if hasattr(root[i], 'perturbation') and root[i].perturbation is not None:
            if isinstance(root[i], BoundParams):
                ...
                if res_dict is not None:
                    res_dict['Final_Backprop_ANS'] = {
                        "lA": lA,
                        "uA": uA,
                        "lbias": lb,
                        "ubias": ub,
                        "center": root[i].center.unsqueeze(0),
                        }
